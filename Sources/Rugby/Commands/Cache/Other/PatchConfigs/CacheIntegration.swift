...
struct CacheIntegration {
    ...
    func replacePathsToCache() throws {
        ...
        try filePatcher.updateFrameworkSearchPaths(inFilesByRegEx: "(\(fileRegex))", folder: supportFilesFolder)
        try filePatcher.fixSwiftInterfaceIfNeeded(for: builtTargets)
    }
}
